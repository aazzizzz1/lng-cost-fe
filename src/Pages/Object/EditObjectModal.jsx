...
const EditObjectModal = ({ isOpen, onClose, onUpdate, sensor }) => {
  ...
  const [baudRate, setBaudRate] = useState(sensor?.baudrate || 1);
  const [updateRate, setUpdateRate] = useState(sensor?.updateRate || 1);
  ...
  useEffect(() => {
    if (sensor) {
      ...
      setPort(sensor.port);
      setCommunication(sensor.communication);
  ...
  return isOpen ? (
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm">
      <section className="bg-white dark:bg-gray-900 p-2 rounded-lg shadow-lg">
