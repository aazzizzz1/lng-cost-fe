...
import ReceivingTerminalTable from './ReceivingTerminalTable'
import { setFilterTipe, setFilterKategori } from '../../../Provider/HargaSatuan/receivingTerminalSlice'

const tipeTabs = [
  { label: "FSRU", value: "FSRU" },
  { label: "ORF", value: "ORF" },
  { label: "OTS", value: "OTS" },
  { label: "ORU", value: "ORU" },
];
...
const ReceivingTerimnalPages = () => {
  const dispatch = useDispatch();
  const { data, filterTipe, filterKategori } = useSelector(state => state.receivingTerminal);
  ...
  React.useEffect(() => {
    if (tab && tabMap[tab.toLowerCase()] && filterTipe !== tabMap[tab.toLowerCase()]) {
      dispatch(setFilterTipe(tabMap[tab.toLowerCase()]));
    }
    ...
    <ErrorBoundary>
      <div className="p-4 dark:bg-darkmode md:h-screen dark:overflow-auto">
        <p className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Receiving Terminal - Harga Satuan
        </p>
        {/* Tabs */}
        ...
        {/* Table */}
        <ReceivingTerminalTable data={filteredData} />
      </div>
    </ErrorBoundary>
  )
}

export default ReceivingTerimnalPages